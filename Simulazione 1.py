...
### BONUS (Mi va di farlo)

# Data una lista di matrici (liste di liste), ordina le matrici in base ai seguenti tre fattori, in ordine di priorità:

# Somma totale degli elementi nella matrice (prima priorità, in ordine crescente).
# Numero di righe nella matrice (seconda priorità, in ordine decrescente).
# Valore massimo nella prima riga della matrice (terza priorità, in ordine crescente).
# Se due matrici sono uguali in base ai primi due criteri, usa il terzo criterio per determinare l'ordine.
...
def func_5(matrix : list[list[list()]]) -> list[list[list()]]:
    return sorted(matrix, key = lambda x :
                  [sum(sum(row) for row in x), 
                   -len(x), 
                   max(x[0]) if x and x[0] else float("-inf")])
    
    
print(func_5(matrici))
